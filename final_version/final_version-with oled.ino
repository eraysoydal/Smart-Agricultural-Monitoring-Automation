...
// ---------- OLED DISPLAY UPDATE ----------
void updateOLED(float press, float tempC, bool pumpIsRunning) {
  u8g2.clearBuffer();
  
  // Title part
  u8g2.setFont(u8g2_font_profont11_tr); 
  u8g2.drawStr(12, 10, "ARAZI KONTROL V2.0");
  u8g2.drawLine(2, 13, 126, 13);

  // Pressure
  char buf[32];
  if (isnan(press)) {
    sprintf(buf, "PRESSURE: WAIT");
  } else {
    sprintf(buf, "PRESSURE: %.2f bar", press);
  }
  ...
  // Pump status
  sprintf(buf, "PUMP: [%s]", pumpIsRunning ? "RUNNING" : "STOPPED");
  u8g2.drawStr(2, 54, buf);

  // Bottom info (RSSI & Blynk status)
  u8g2.drawLine(2, 57, 126, 57);
  u8g2.setFont(u8g2_font_4x6_tr);
  
  long rssi = WiFi.status() == WL_CONNECTED ? WiFi.RSSI() : 0;
  bool blynkState = Blynk.connected();
  
  sprintf(buf, "RSSI: %lddBm | BLYNK: %s", rssi, blynkState ? "ONLINE" : "OFFLINE");
  
  // Center or approximate center for bottom line
...
void readAndMaybeSendAll() {
  ...
  // Update OLED view
  // avgNow is the smoothed 5-second pressure. If it's NaN we can fallback to pressureVal.
  float displayPressure = !isnan(avgNow) ? avgNow : pressureVal;
  updateOLED(displayPressure, tempC, sondajControlState);
}
